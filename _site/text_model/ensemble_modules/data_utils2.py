...
class H5Dataset_ensemble(Dataset):
    """
    ...
    """
    def __init__(self, path, data_transforms, tokenizer, apply_cleaning, max_tokenization_length,
                 truncation_method='head-only', split_head_density=0.5, device='cpu', phase='train'):
        super(H5Dataset_ensemble).__init__()
        self.file_path = path
        self.dataset = None
        self.data = None
        self.target = None
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.from_numpy(np.array(example)).long().to(self.device), label
