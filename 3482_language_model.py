...
# hdf5_path = './HDF5_files/hdf5_small_tobacco_papers_audebert_800.hdf5'
...
num_samples_train = 600#800
num_samples_val = 800#1000
...
def extract_ocrs(ocrs):
    for i in range(len(ocrs)):
        ...
        if ocrs ==val_ocrs:
            corpus = open("small_tobacco_corpus_test.txt", "a")
            corpus.write(text)
            corpus.close()
        else:
            corpus = open("small_tobacco_corpus_train.txt", "a")
            corpus.write(text)
            corpus.close()
